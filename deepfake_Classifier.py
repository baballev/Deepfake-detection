...
class RNN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ToDo: also test with the mean of all the frames output
        return y
...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        images = []
        for i in indices:
            ## Change the 'number_of_frames' criteria if you get OOM errors.
...
validloader = torch.utils.data.DataLoader(validset, batch_size=4, sampler=valid_sampler)
...
def trainModel(model, loss_function, optimizer, epochs_nb, scheduler):
    ...
    for epoch in range(epochs_nb):
        ...
        for i, data in enumerate(trainloader):
            ...
            print("Video " + str(i) + " / " + str(train_size))
            running_loss += loss.item()
            running_corrects += torch.sum(predicted == label.detach()).item()
            verbose_loss += loss.item()
            if i% 100 == 0 and i !=0:
                print("Loss over last 100 videos: " + str(verbose_loss/100))
                verbose_loss = 0.0
            scheduler.step()
...
trainModel(rnn, loss_function, optimizer, 3, exp_lr_scheduler)
...
save_path = 'E:/Programmation/Python/PAF 2020/deepfake2/weights/30-06-2020-ResNet18&3LSTM-7epochs.pth'
